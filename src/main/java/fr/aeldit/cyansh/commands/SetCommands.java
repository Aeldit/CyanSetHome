...
public class SetCommands
{
     ...
     * </ul>
     */
    public static int setAllowSethome(@NotNull CommandContext<ServerCommandSource> context)
    {
        ...
        if (player.hasPermissionLevel(SethomeMidnightConfig.minOpLevelExeKgi))
        {
            ...
            player.sendMessage(Text.translatable("sethome.message.setAllowSethome", Boolean.toString(arg)), true);
        }
        // If not OP or not OP with max level
        else
        {
            source.sendFeedback(Text.translatable("sethome.message.notOp"), true);
        }
     ...
     * </ul>
     */
    public static int setAllowHomeOf(@NotNull CommandContext<ServerCommandSource> context)
    {
        ServerCommandSource source = context.getSource();
        ServerPlayerEntity player = source.getPlayer();
        boolean arg = BoolArgumentType.getBool(context, "bool");
        ...
        if (player.hasPermissionLevel(SethomeMidnightConfig.minOpLevelExeKgi))
        {
            ...
            player.sendMessage(Text.translatable("sethome.message.setAllowHomeOf", Boolean.toString(arg)), true);
        }
        // If not OP or not OP with max level
        else
        {
     ...
     * </ul>
     */
    public static int setMaxHomes(@NotNull CommandContext<ServerCommandSource> context)
    {
        ServerCommandSource source = context.getSource();
        ServerPlayerEntity player = source.getPlayer();
        ...
        if (arg < 1 || arg > 64)
        {
            player.sendMessage(Text.translatable("sethome.message.incorrectIntMaxHomes"), false);
            return 0;
        }
        ...
        if (player.hasPermissionLevel(SethomeMidnightConfig.minOpLevelExeKgi))
        {
            ...
            player.sendMessage(Text.translatable("sethome.message.setMaxHomes", arg), true);
        }
        // If not OP or not OP with max level
        else
        {
    ...
    public static int setRequiredOpLevelKgi(@NotNull CommandContext<ServerCommandSource> context)
    {
        ...
        if (arg < 0 || arg > 4)
        {
            player.sendMessage(Text.translatable("sethome.message.incorrectIntOp"), false);
            return 0;
        }
        ...
        if (player.hasPermissionLevel(SethomeMidnightConfig.minOpLevelExeKgi))
        {
            ...
            player.sendMessage(Text.translatable("sethome.message.setRequiredOpLevelKgi", arg), true);
        }
